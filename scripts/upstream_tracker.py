...
# ── HTTP ────────────────────────────────────────────────────────────────
def gh_get(url: str, timeout: int = 15, retries: int = 2) -> tuple[dict | list | None, bool]:
    """Fetch JSON from GitHub API with retry logic. Returns (data, ok).
    ...
    req = Request(url, headers={
        ...
        "User-Agent": "hermes-upstream-tracker/1.2",
    })
        ...
        try:
            ...
            if e.code == 403:
                ...
                if attempt <= retries:
                    ...
                    print(f"   ⚠️  GitHub API 限速 (403)，等待 {wait_time}s 后重试 ({attempt}/{retries})...")
                    time.sleep(wait_time)
                else:
                    print(f"   ❌ GitHub API 限速，已重试 {retries} 次")
                    return None, False
            elif e.code == 404:
                print(f"   ❌ 资源未找到 (404): {url[:80]}")
                return None, False
            else:
                print(f"   ❌ HTTP 错误 {e.code}: {url[:80]}")
                return None, False
        except (URLError, TimeoutError) as e:
            attempt += 1
            if attempt <= retries:
                wait_time = 10 * attempt
                print(f"   ⚠️  网络错误，等待 {wait_time}s 后重试 ({attempt}/{retries})...")
                time.sleep(wait_time)
            else:
                print(f"   ❌ 网络错误，已重试 {retries} 次：{type(e).__name__}")
                return None, False
...
def fetch(url: str, timeout: int = 15, retries: int = 2) -> str | None:
    ...
    """
    req = Request(url, headers={"User-Agent": "hermes-upstream-tracker/1.2"})
    ...
    while attempt <= retries:
        try:
            ...
            if attempt <= retries:
                ...
                print(f"   ❌ 无法获取文件，已重试 {retries} 次：{url[:80]}")
                return None
        except Exception as e:
            print(f"   ❌ 获取失败：{type(e).__name__}: {url[:80]}")
            return None
