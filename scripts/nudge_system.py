...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="智能 Nudge 系统 - 基于任务复杂度和时间自动触发提醒",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python nudge_system.py --check
  python nudge_system.py --trigger learning
  python nudge_system.py --config
  python nudge_system.py --stats
        """
    )
    parser.add_argument("--check", action="store_true", help="检查所有 nudges")
    parser.add_argument("--trigger", type=str, help="触发特定 nudge (learning/reflection/cleanup/memory)")
    ...
    if args.check:
        ...
        valid_triggers = ["learning", "reflection", "cleanup", "memory"]
        if args.trigger not in valid_triggers:
            print(f"❌ 无效的 nudge 类型：{args.trigger}", file=sys.stderr)
            print(f"可用类型：{', '.join(valid_triggers)}", file=sys.stderr)
            return 1
