...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="技能评估系统 - 自动评估和改进 skills",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python skill_evaluator.py --evaluate my-skill,4,反馈内容
  python skill_evaluator.py --improve my-skill,改进说明
  python skill_evaluator.py --stats
  python skill_evaluator.py --suggest my-skill
        """
    )
    parser.add_argument("--evaluate", type=str, help="评估 skill: name,rating[,feedback]")
    ...
    if args.evaluate:
        ...
        if len(parts) < 2:
            print("❌ 格式错误：应为 skill_name,rating[,feedback]", file=sys.stderr)
            print("示例：--evaluate my-skill,4,很棒的技能", file=sys.stderr)
            sys.exit(1)
        ...
        try:
            ...
            if rating < 1 or rating > 5:
                print("❌ 评分必须在 1-5 之间", file=sys.stderr)
                sys.exit(1)
        except ValueError:
            print("❌ 评分必须是整数", file=sys.stderr)
            sys.exit(1)
        ...
        if len(parts) < 2:
            print("❌ 格式错误：应为 skill_name,notes", file=sys.stderr)
            print("示例：--improve my-skill,增加错误处理", file=sys.stderr)
            sys.exit(1)
